...
let $response := http:send-request(<http:request
  method="post"
  href="https://spacyapp.acdh-dev.oeaw.ac.at/query/enrich-simple-json/">
    { $request-headers }
    <http:body media-type="application/json">{$tokens}</http:body>
  </http:request>

let $code := $response//http:body/@encoding

let $ann := if ($response//http:header[@name = 'Content-Type']/@value = "application/json; charset=utf-8")
    then util:base64-decode($response//http:body)
    else ""
